			...
			if(testingDes.config.statusType == "normal"){
				...
				for (var i = this.parts.length - 1; i >= 0; i--) {
					...
					for (var ti = part.items.length - 1; ti >= 0; ti--) {
						...
						if(Player.paperAnswers[itemSubjectId]){
							//如果此大题已经作答了，从此大题开始
							ended = true;
...
//		console.log(this.pageIndex);
	...
	back: function(){
		...
		if(back != null){
			...
			this.sendAnswer({partIndex: this.partIndex, itemIndex: back.itemIndex, pageIndex: back.pageIndex}); //发送当前页的答案
...
var Review = {
		...
		for (var pi = 0; pi < Player.parts.length; pi++) {
			...
			for (var ti = 0; ti < part.items.length; ti++) {
				...
				for (var pageInd = 0; pageInd < subjectList.pages.length; pageInd++) {
					...
					if(page.isExplain == "false"){
						//是题目页
						var subjectInfo = this.getPageSubjectInfo(page);
						var sub = {
							partIndex: pi,
							itemIndex: ti,
							pageIndex: pageInd,
	...
	buildList: function(partResults){
		...
		for (var i = 0; i < partResults.length; i++) {
			...
			for (var si = 0; si < result.subjects.length; si++) {
				...
				var subItem = $("<li class='item'><div class='review_no'>"+(si+1)+"</div><div class='review_des'>"+sub.des+"</div><div class='review_uanswer'> </div><div class='review_canswer'>"+sub.rightAns+"</div><div class='review_correct'><span class='tpbtn'>未批改</span></div></li>")
	 ...
	 * 获取一页题的题目信息
	 */
	getPageSubjectInfo: function(page){
		var result = {
			des: "",
			mark: 0,
			rightAns: ""
